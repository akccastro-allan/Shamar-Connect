...
function Stat({ label, value }: { label: string; value: React.ReactNode }) {
  return <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4"><p className="text-xs font-black uppercase tracking-wide text-slate-500">{label}</p><p className="mt-1 break-words text-lg font-black text-[#1B2F5B]">{value || "—"}</p></div>;
}

function SnapshotCards({ snapshot }: { snapshot: Snapshot }) {
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <Card className="rounded-[2rem]"><CardHeader><CardTitle className="text-lg font-black text-[#1B2F5B]">Status da sessão</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2"><Stat label="Empresa" value="Auto Peças e Auto Center Lips" /><Stat label="Canal" value={snapshot.channel.sessionId} /><Stat label="Provider" value="OpenWA" /><Stat label="Ambiente" value="Preview" /><Stat label="Modo" value="Homologação interna" /><Stat label="Conectado" value={snapshot.connection.connected ? "sim" : "não"} /></CardContent></Card>
      <Card className="rounded-[2rem]"><CardHeader><CardTitle className="text-lg font-black text-[#1B2F5B]">Estado da sincronização</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2"><Stat label="Sync" value={snapshot.state?.syncStatus || "idle"} /><Stat label="Status provider" value={snapshot.connection.providerStatus || "não consultado"} /><Stat label="Último sucesso" value={snapshot.state?.lastSuccessAt || "nunca"} /><Stat label="Último erro seguro" value={snapshot.state?.lastError || "—"} /><Stat label="Último modo" value={snapshot.state?.lastMode || "—"} /><Stat label="Próxima reconciliação" value={snapshot.state?.nextReconciliationAt || "—"} /></CardContent></Card>
    ...
    <div className="space-y-8">
      <header className="rounded-[2rem] bg-[#1B2F5B] p-7 text-white shadow-sm md:p-9">
        <div className="flex flex-wrap items-center gap-2 text-sm font-bold text-white/70">
          <Link href="/operations" className="hover:text-white">Centro de Comando</Link><span>→</span><span>Diagnósticos</span><span>→</span><span className="text-white">Sincronização WhatsApp</span>
        </div>
        <div className="mt-5 flex flex-wrap items-center gap-2"><Badge className="border-white/20 bg-white/10 text-white">Uso interno</Badge><Badge className="border-white/20 bg-white/10 text-white">Ambiente: {vercelEnv === "preview" ? "Preview" : vercelEnv}</Badge><Badge className="border-white/20 bg-white/10 text-white">Modo: Homologação interna</Badge></div>
        <h1 className="mt-4 text-3xl font-black tracking-tight md:text-5xl">Sincronização WhatsApp</h1>
        <p className="mt-4 max-w-4xl text-sm leading-6 text-slate-200 md:text-base">Diagnóstico interno dos canais e da sincronização automática.</p>
        <div className="mt-5 rounded-2xl border border-white/15 bg-white/10 p-4 text-sm font-black text-white">Esta ferramenta não envia mensagens para clientes.</div>
      </header>
