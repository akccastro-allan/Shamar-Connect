...
export function AdminSubscriptionsClient() {
  ...
  function openPlanEdit(sub: Sub) {
    setPlanForm({
      ...
      total_amount: String(sub.total_amount),
      reason: "",
    });
              ...
              {subs.map((sub) => (
                <tr key={sub.id}>
                  <td className="px-5 py-4">
                    <p className="text-xs font-bold text-slate-800">{sub.organization_id.slice(0, 8)}…</p>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex flex-wrap gap-2">
                      {sub.status === "active" && (
                        <>
                          <Button size="sm" variant="outline" disabled={acting === sub.id} onClick={() => changeStatus(sub.id, "paused", "Pausar")}>
                            Pausar
                          </Button>
                          <Button size="sm" variant="outline" className="border-red-200 text-red-700 hover:bg-red-50" disabled={acting === sub.id} onClick={() => changeStatus(sub.id, "cancelled", "Cancelar")}>
                            Cancelar
                          </Button>
                        </>
                      )}
                      ...
                      {sub.status === "paused" && (
                        <>
                          <Button size="sm" className="bg-emerald-700 hover:bg-emerald-800" disabled={acting === sub.id} onClick={() => changeStatus(sub.id, "active", "Reativar")}>
                            Reativar
                          </Button>
                          <Button size="sm" variant="outline" className="border-red-200 text-red-700 hover:bg-red-50" disabled={acting === sub.id} onClick={() => changeStatus(sub.id, "cancelled", "Cancelar")}>
                            Cancelar
                      ...
                      <Button size="sm" variant="outline" disabled={acting === sub.id} onClick={() => (editingPlan === sub.id ? setEditingPlan(null) : openPlanEdit(sub))}>
                    ...
                    {editingPlan === sub.id && (
                      <div className="mt-3 space-y-2 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                        <div className="flex gap-2">
                          <select value={planForm.plan_slug} onChange={(e) => setPlanForm((f) => ({ ...f, plan_slug: e.target.value }))} className="flex-1 rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs">
                            <option value="starter">Essencial</option>
                            <option value="professional">Professional</option>
                            <option value="business">Business</option>
                          </select>

                          <select value={planForm.billing_cycle} onChange={(e) => setPlanForm((f) => ({ ...f, billing_cycle: e.target.value }))} className="flex-1 rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs">
                            <option value="monthly">Mensal</option>
                            <option value="annual">Anual</option>
                          </select>
                        </div>

                        <input type="text" placeholder="Valor (R$)" value={planForm.total_amount} onChange={(e) => setPlanForm((f) => ({ ...f, total_amount: e.target.value }))} className="w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs" />

                        <input type="text" placeholder="Motivo (opcional)" value={planForm.reason} onChange={(e) => setPlanForm((f) => ({ ...f, reason: e.target.value }))} className="w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs" />

                        <Button size="sm" className="w-full bg-[#1B2F5B] hover:bg-[#1B2F5B]/90" disabled={acting === sub.id} onClick={() => savePlan(sub.id)}>
