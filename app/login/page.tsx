  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#F8FAFC] px-6 py-10 text-slate-900">
      <section className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <a href="/api/auth/oauth?provider=google" className="mt-8 flex w-full justify-center rounded-xl bg-[#2ABFAB] px-4 py-3 font-semibold text-white hover:opacity-90">
          Entrar com Google
        </a>
        ...
        <form action="/api/auth/login" method="post" className="space-y-4">
          ...
          <div className="text-right">
            <Link href="/forgot-password" className="text-sm font-medium text-[#1B2F5B] hover:text-[#2ABFAB]">
              Esqueci minha senha
            </Link>
          </div>

          <button className="w-full rounded-xl bg-[#1B2F5B] px-4 py-3 font-semibold text-white hover:opacity-90">
            Entrar
          </button>
        </form>

        <footer className="mt-8 text-center text-xs text-slate-400">
          © 2026 Moriah Systems Serviços Ltda
        </footer>
      </section>
    </main>
  );
