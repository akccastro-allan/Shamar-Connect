...
const features = [
  [
    "WhatsApp Central",
    "Gerencie todas as conversas em um único painel, sem perder mensagens importantes.",
  ],
  [
    "CRM Completo",
    "Cadastre contatos, acompanhe o funil e registre oportunidades de venda.",
  ],
  [
    "Respostas Rápidas",
    "Crie modelos de atendimento e responda clientes em segundos.",
  ],
  [
    "IA Assistida",
    "Use sugestões, resumos e transcrições para acelerar o atendimento.",
  ],
];
...
const plans = [
  ...
  [
    "Professional",
    "R$ 197/mês",
    "Para equipes com multiatendimento e funil completo.",
  ],
  [
    "Business",
    "R$ 397/mês",
    "Para empresas que precisam de automações, catálogo e Agent.",
  ],
];
...
const faqs = [
  ...
  [
    "Posso ter mais de um atendente?",
    "Sim. Os planos permitem equipes de atendimento conforme a necessidade.",
  ],
  [
    "O sistema tem IA?",
    "Sim. O módulo de IA pode apoiar respostas, resumos e transcrição de áudio.",
  ],
  [
    "Posso cancelar quando quiser?",
    "Sim. A proposta é simples e sem fidelidade obrigatória.",
  ],
];
      ...
      <section className="mx-auto grid max-w-6xl items-center gap-10 px-6 py-16 md:grid-cols-2 md:py-24">
        <div>
          ...
          <p className="mt-6 text-lg leading-8 text-slate-600">
            O ShamarConnect centraliza conversas, contatos, oportunidades e
            automações em um painel simples para sua equipe vender melhor.
          </p>

          <div className="mt-8 flex flex-wrap gap-4">
            <Link
              href="/login"
              className="rounded-xl bg-[#2ABFAB] px-6 py-3 font-semibold text-white shadow-sm hover:opacity-90"
            >
              Começar agora
            </Link>
            ...
            <Link
              href="#planos"
              className="rounded-xl border border-[#1B2F5B] px-6 py-3 font-semibold text-[#1B2F5B] hover:bg-white"
            >
              Ver planos
            </Link>
          </div>
        ...
        <div className="rounded-3xl bg-white p-6 shadow-sm">
          ...
          <div className="mt-5 space-y-3">
            {["Novo lead recebido", "Orçamento enviado", "Cliente pronto para fechar"].map(
              (item) => (
                <div
                  key={item}
                ...
                >
                  ...
                  <span className="rounded-full bg-[#2ABFAB]/10 px-3 py-1 text-xs font-semibold text-[#2ABFAB]">
                    ativo
                  </span>
                </div>
              )
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {steps.map((step, index) => (
              <article key={step} className="rounded-2xl bg-[#F8FAFC] p-6">
                <span className="text-sm font-bold text-[#2ABFAB]">
                  Passo {index + 1}
                </span>
                <h3 className="mt-3 text-xl font-bold text-[#1B2F5B]">
                  {step}
                </h3>
              </article>
            ))}
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-3">
          {plans.map(([name, price, text]) => (
            <article key={name} className="rounded-3xl bg-white p-8 shadow-sm">
              ...
              <p className="mt-4 text-3xl font-bold text-[#2ABFAB]">
                {price}
              </p>
              ...
              <Link
                href="/login"
                className="mt-6 inline-flex rounded-xl bg-[#1B2F5B] px-5 py-3 font-semibold text-white hover:opacity-90"
              >
                Começar
              </Link>
            </article>
