...
export const OFFICIAL_QUEUE_STATUSES: QueueStatus[] = ["new", "queued", "assigned", "in_progress", "awaiting_customer", "pending_internal", "resolved", "closed"];
export const ACTIVE_QUEUE_STATUSES = ["queued", "assigned", "in_progress", "awaiting_customer", "pending_internal"];

export function isSupervisorRole(role: AppContext["role"] | string) {
  return role === "owner" || role === "admin";
}

export function slaStatusFromDueAt(dueAt?: string | null, status?: string | null) {
  if (!dueAt) return status === "completed" ? "completed" : "on_time";
  if (status === "completed") return "completed";
  const due = new Date(dueAt).getTime();
  if (Number.isNaN(due)) return "on_time";
  return Date.now() > due ? "breached" : "on_time";
}
...
export async function assertConversationAccess(db: QueueDb, context: AppContext, conversationId: string) {
  const { data, error } = await db
    ...
    .select("id, tenant_id, organization_id, channel_id, is_group, status, assigned_to, department_id, requires_human, pending_reason, queue_entered_at, last_assigned_at, last_assigned_user_id, resolved_at, channels(session_id)")
...
export async function selectAvailableAssignee(db: QueueDb, input: { tenantId: string; organizationId: string; departmentId: string }) {
  ...
  const { data: availability, error: availabilityError } = await db
    ...
    .select("app_user_id, status, capacity, active_conversations")
    .eq("organization_id", input.organizationId)
  ...
  const available = (availability ?? [])
    .filter((item) => Number(item.active_conversations ?? 0) < Number(item.capacity ?? 1))
    .map((item) => ({
      appUserId: item.app_user_id as string,
      active: Number(item.active_conversations ?? 0),
      lastAssignedAt: memberships?.find((membership) => membership.app_user_id === item.app_user_id)?.last_assigned_at ?? null,
    }))
    .sort((a, b) => a.active - b.active || new Date(a.lastAssignedAt || 0).getTime() - new Date(b.lastAssignedAt || 0).getTime());
...
export async function routeAndQueueConversation(db: QueueDb, context: AppContext, input: {
  ...
  const now = new Date();
  const autoAssignee = decision.preserveCurrentAssignment && conversation.assigned_to ? conversation.assigned_to : await selectAvailableAssignee(db, { tenantId: context.tenantId, organizationId: context.organizationId, departmentId });
  const nextStatus = autoAssignee ? "assigned" : "queued";
  const patch = {
    ...
    sla_status: decision.slaMinutes === 0 ? "breached" : "on_time",
  ...
  const { data, error } = await db
    ...
    .eq("tenant_id", context.tenantId)
    .eq("organization_id", context.organizationId)
    .select("id, status, department_id, priority, sla_due_at, sla_status")
    .single();
  if (error) throw error;
